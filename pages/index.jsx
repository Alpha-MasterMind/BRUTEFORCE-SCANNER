...
// ═══════════════════════════════════════════════════════
// DOMAIN INTELLIGENCE — BUILT-IN LISTS
// ═══════════════════════════════════════════════════════

const MOYA_DOMAINS = [
  // Core moya.app domains
...
const SA_ZERO_RATED_GOV = [
  // Government — official zero-rated
...
const SA_MISC_ZERO_RATED = [
  // News / social / other confirmed zero-rated
...
const PORT_RISK: Record<number, number> = {
...
export default function Scanner() {
  const [tab, setTab] = useState("sni"); // sni | recon | history
  ...
  async function loadHistory() {
    try {
      const r = await fetch('/functions/getJobs');
  ...
  async function launchSNI() {
    ...
    let job;
    try {
      const cj1 = await fetch('/functions/createJob', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ status: "pending", mode: "pattern", input: `SNI: ${sniPreset || "custom"} (${hosts.length} hosts)`, ports: sniPort, concurrency: 25 }) });
      const cd1 = await cj1.json();
      if (cd1.error) { setSniError(cd1.error); return; }
      job = cd1.job;
    } catch (e) { setSniError(e.message); return; }
    ...
    pollRef.current = setInterval(async () => {
      try {
        const pj = await fetch(`/functions/getJob?id=${job.id}`); const j = await pj.json();
        if (j.results) {
          const parsed = JSON.parse(j.results);
          setSniResults(parsed);
          setSniTested(parsed.length * 3);
        }
        if (j.status === "done" || j.status === "error") {
          clearInterval(pollRef.current);
          setSniStatus(j.status);
          if (j.results) setSniResults(JSON.parse(j.results));
          loadHistory();
        }
      } catch {}
    }, 2000);

    fetch("/functions/sniScan", {
      method: "POST",
      ...
      const d = await r.json();
      if (d.error) { setSniError(d.error); setSniStatus("error"); clearInterval(pollRef.current); }
      else { setSniResults(d.results || []); setSniStatus("done"); setSniTested(d.tested || 0); clearInterval(pollRef.current); loadHistory(); }
    }).catch(e => { setSniError(e.message); setSniStatus("error"); clearInterval(pollRef.current); });
  }
  ...
  async function launchRecon() {
    ...
    const portStr = PORT_PROFILES[reconProfile];
    const ports = portStr.split(",").map(s => parseInt(s.trim())).filter(n => !isNaN(n));
    const wordlist = ["www","mail","api","dev","test","stage","prod","vpn","admin","portal","auth","secure","cdn","docs","status","billing","dashboard","support","app","shop"];
    const targets = [];
    for (const d of domains) { targets.push(d); for (const s of wordlist) targets.push(`${s}.${d}`); }

    let job;
    try {
      const cj2 = await fetch('/functions/createJob', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ status: "pending", mode: "wordlist", input: reconDomains, ports: portStr, concurrency: 30 }) });
      const cd2 = await cj2.json();
      if (cd2.error) { setReconError(cd2.error); return; }
      job = cd2.job;
    } catch (e) { setReconError(e.message); return; }
    ...
    pollRef.current = setInterval(async () => {
      try {
        const pj = await fetch(`/functions/getJob?id=${job.id}`); const j = await pj.json();
        if (j.results) { setReconResults(JSON.parse(j.results)); setReconProgress(j.status === "done" ? 100 : 60); }
        if (j.status === "done" || j.status === "error") {
          clearInterval(pollRef.current); setReconStatus(j.status); setReconProgress(100); loadHistory();
        }
      } catch {}
    }, 1500);

    fetch("/functions/realtimeScan", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ job_id: job.id, domains: targets, profile: reconProfile, ports }),
    }).then(async r => {
      const d = await r.json();
      if (d.error) { setReconError(d.error); setReconStatus("error"); clearInterval(pollRef.current); }
      else { setReconResults(d.results || []); setReconStatus("done"); setReconProgress(100); clearInterval(pollRef.current); loadHistory(); }
    }).catch(e => { setReconError(e.message); setReconStatus("error"); clearInterval(pollRef.current); });
  }

  function loadHistoryJob(j) {
    clearInterval(pollRef.current);
    if (!j.results) return;
    try {
      const parsed = JSON.parse(j.results);
      if (j.mode === "pattern") { setSniResults(parsed); setTab("sni"); setSniStatus(j.status); }
      else { setReconResults(parsed); setTab("recon"); setReconStatus(j.status); setReconProgress(100); }
    } catch {}
  }
      ...
      <div className="max-w-screen-xl mx-auto px-5 py-5 space-y-4">
        ...
        {tab === "history" && (
          <div className="bg-white/4 border border-white/10 rounded-2xl overflow-hidden">
            ...
            {history.length === 0
              ? <div className="py-16 text-center text-white/15 text-sm">No scans yet.</div>
              : (
                <div className="divide-y divide-white/5">
                  {history.map(j => (
                    <div key={j.id} onClick={() => loadHistoryJob(j)}
                      className="px-5 py-3.5 hover:bg-white/5 cursor-pointer transition flex items-center justify-between group">
                      <div>
                        <div className="font-mono text-white/60 text-xs truncate max-w-xl">{j.input}</div>
                        <div className="text-[10px] text-white/20 mt-1">{new Date(j.created_date).toLocaleString()}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        {j.total_live != null && <span className="font-mono text-xs text-cyan-400">{j.total_live} found</span>}
                        <Chip color={j.status === "done" ? "green" : j.status === "running" ? "yellow" : j.status === "error" ? "red" : "gray"}>
                          {j.status}
                        </Chip>
                        <Chip color="gray">{j.mode}</Chip>
                      </div>
                    </div>
                  ))}
                </div>
