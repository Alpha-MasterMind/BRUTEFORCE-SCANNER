...
// ─── Subcomponents (each defined ONCE) ───────────────────────────────
...
export default function Scanner() {
  ...
  // ─── Proxy Test (Multi‑Port) ──────────────────────────────────────
  async function testProxy() {
    if (!proxyHost) {
      setProxyResult({ error: "Host required" });
      return;
    }
    const ports = proxyPort.trim()
      ? proxyPort.split(',').map(p => parseInt(p.trim())).filter(p => !isNaN(p))
      : [3128, 8888, 9090, 1080, 8118];
    ...
    try {
      const res = await fetch('/api/proxyTest', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ proxyHost, ports, targetUrl: proxyTarget })
      });
        ...
        {tab === "proxy" && (
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/4 border border-white/10 rounded-2xl p-5 space-y-4">
              <div className="text-[10px] text-white/25 uppercase tracking-widest">Proxy Tester (Multi‑Port)</div>
              <div>
              ...
              <div>
                <label className="text-[10px] text-white/20 uppercase block mb-1">Ports (comma‑separated)</label>
                <input value={proxyPort} onChange={e => setProxyPort(e.target.value)}
                  placeholder="3128,8888,9090,1080,8118"
                  className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-xs font-mono text-white focus:outline-none focus:border-cyan-500/40" />
              </div>
              <div>
              ...
              <button onClick={testProxy} disabled={proxyLoading}
                ...
                {proxyLoading ? "TESTING..." : "▶ TEST ALL PORTS"}
              </button>
              {proxyResult && (
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  <div className="text-xs text-white/50">Results for {proxyResult.host}:</div>
                  {proxyResult.results.map((r, i) => (
                    <div key={i} className={`p-3 rounded-lg border ${r.success ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${r.success ? 'bg-green-400' : 'bg-red-400'}`} />
                        <span className="text-xs font-mono">Port {r.port}</span>
                        {r.status && <span className="text-[10px] text-white/40">({r.status})</span>}
                      </div>
                      {r.success && r.data ? (
                        <pre className="text-green-300 text-[10px] mt-1 overflow-x-auto">
                          {JSON.stringify(r.data, null, 2)}
                        </pre>
                      ) : (
                        <div className="text-[10px] text-red-300 mt-1">{r.error || 'Failed'}</div>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white/4 border border-white/10 rounded-2xl p-5 space-y-3">
              ...
              <ol className="text-xs text-white/40 space-y-2 list-decimal list-inside">
                <li>Enter the proxy host (e.g., from Recon results).</li>
                <li>Specify ports to test (default: 3128,8888,9090,1080,8118).</li>
                <li>Target URL defaults to httpbin.org/ip.</li>
                <li>Click "Test All Ports".</li>
                <li>Green = responsive, Red = failed/timeout.</li>
                <li className="text-yellow-400/60">⚠️ Only test on authorized networks.</li>
              </ol>
              <div className="text-[10px] text-white/25 uppercase tracking-widest mt-4">Interpretation</div>
              <ul className="text-[10px] text-white/40 space-y-1 list-disc list-inside">
                <li><span className="text-green-300">HTTP 200 + IP address</span> → Working open proxy.</li>
                <li><span className="text-yellow-300">HTTP 400/403</span> → Proxy reachable but may require auth or is a captive portal.</li>
                <li><span className="text-red-300">Timeout / Connection refused</span> → Port closed or blocked by Vercel.</li>
              </ul>
            </div>
          </div>
        )}
