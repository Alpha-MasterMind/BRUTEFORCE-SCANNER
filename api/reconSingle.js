...
async function resolveDomain(hostname) {
  try { return await dns.resolve4(hostname); } catch { return []; }
}
...
async function scanPorts(ip, ports) {
  ...
  const open = [];
  results.forEach((r, i) => { if (r.status === 'fulfilled' && r.value) open.push(ports[i]); });
  return open;
}
...
export default async function handler(req, res) {
  ...
  for (const target of targets) {
    const targetPromise = (async () => {
      ...
      return { domain: target, ips, open_ports: openPorts, proxy_ok: proxyOk, highlight: proxyOk || openPorts.length > 5, score, http, geo };
    })();

    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('target_timeout')), 10000));
