...
// Helper: HTTP probe (with SNI if different from IP)
async function httpProbe(host, port = 443) {
  ...
  try {
    ...
    // HEAD doesn't return body, so we can't get title easily; skip for now
    return {
      status: res.status,
      server,
      title: null
...
// Helper: GeoIP using free ip-api.com
